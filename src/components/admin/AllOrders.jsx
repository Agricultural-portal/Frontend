...
export function AllOrders() {
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const matchesSearch =
      ...
      order.buyer?.email?.toLowerCase().includes(searchString) ||
      order.buyer?.firstName?.toLowerCase().includes(searchString) ||
      order.buyer?.lastName?.toLowerCase().includes(searchString) ||
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: "Cancelled", value: stats.CANCELLED, color: "text-red-600", status: "CANCELLED" },
        ].map((stat, i) => (
          <Card 
            key={i} 
